...
u8 changeAddress(u8 oldAddress, u8 newAddress){
  ...
  if(I2C_Start()==0)  //起始信号
    ...
    I2C_SendByte(oldAddress);   //发送设备地址+写信号
 	  if(!I2C_WaitAck()){I2C_Stop(); return RESET;}
		//	delay_1us(40); 
		I2C_SendByte(0xaa);    //指令1
		 if(!I2C_WaitAck()){I2C_Stop(); return RESET;}
		I2C_SendByte(0xa5);    //指令2
	  if(!I2C_WaitAck()){I2C_Stop(); return RESET;}
		I2C_SendByte(newAddress);    //新地址
 	  if(!I2C_WaitAck()){I2C_Stop(); return RESET;}
...
u8 changemode(u8 Address, u8 mode,u8 time){
  ...
  if(I2C_Start()==0)  //起始信号
    ...
    I2C_SendByte(Address);   //发送设备地址+写信号
 	  if(!I2C_WaitAck()){I2C_Stop(); return RESET;}
		//	delay_1us(40); 
		I2C_SendByte(0x5a);    //指令1
		 if(!I2C_WaitAck()){I2C_Stop(); return RESET;}
		I2C_SendByte(0x55);    //指令2
	  if(!I2C_WaitAck()){I2C_Stop(); return RESET;}
		I2C_SendByte(temp);    //配置模式和时间
 	  if(!I2C_WaitAck()){I2C_Stop(); return RESET;}
